...
const GridDistortion = ({ grid = 15, mouse = 0.1, strength = 0.15, relaxation = 0.9, imageSrc, className = '' }) => {
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (!currentContainer || !currentRenderer || !currentCamera || !currentPlane) return;
      ...
      let imageAspect = imageAspectRef.current;
      ...
      currentPlane.scale.set(planeWidth, planeHeight, 1);

      currentCamera.left = -width / 2;
      currentCamera.right = width / 2;
      currentCamera.top = height / 2;
      currentCamera.bottom = -height / 2;
    ...
    const animate = () => {
      ...
      if (!currentRenderer || !currentScene || !currentCamera || !uniforms.uDataTexture.value) return;

      uniforms.time.value += 0.05;

      const data = uniforms.uDataTexture.value.image.data;
      for (let i = 0; i < size * size; i++) {
        data[i * 4] *= relaxation;
